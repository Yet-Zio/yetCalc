...
class EnergyFragment : Fragment() {
            ...
            try {
                when(id){
                    R.id.et_firstConversion -> {
                        val res = UnitConv.Energy.convert(pViewModel._energyftpos, pViewModel._energysdpos
                    ...
                    R.id.et_secondConversion -> {
                        val res = UnitConv.Energy.convert(pViewModel._energysdpos, pViewModel._energyftpos
    ...
    fun setupSpinner(){
        activity?.let {
            if(pLight){
                ArrayAdapter.createFromResource(it, R.array.energylist, R.layout.spinner_itemlight)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
            else{
                ArrayAdapter.createFromResource(it, R.array.energylist, R.layout.spinner_item)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
