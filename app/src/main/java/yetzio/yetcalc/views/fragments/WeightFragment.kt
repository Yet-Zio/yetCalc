...
class WeightFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val initPos1 = prefMgr?.getInt("wmft", 0)!!
        val initPos2 = prefMgr?.getInt("wmsd", 0)!!
        spinner?.setSelection(initPos1)
        spinner2?.setSelection(initPos2)
        pViewModel._wmftpos = initPos1
        pViewModel._wmsdpos = initPos2
            ...
            try {
                when(id){
                    R.id.et_firstConversion -> {
                        val res = UnitConv.WeightORMass.convert(pViewModel._wmftpos, pViewModel._wmsdpos
                    ...
                    R.id.et_secondConversion -> {
                        val res = UnitConv.WeightORMass.convert(pViewModel._wmsdpos, pViewModel._wmftpos
    ...
    fun setupSpinner(){
        activity?.let {
            if(pLight){
                ArrayAdapter.createFromResource(it, R.array.weightormasslist, R.layout.spinner_itemlight)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
            else{
                ArrayAdapter.createFromResource(it, R.array.weightormasslist, R.layout.spinner_item)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
        ...
        spinner?.onItemSelectedListener = (object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, vw: View?, pos: Int, id: Long) {
                if(!pViewModel._wmspinInit){
                    pViewModel._wmspinInit = true
                    getConversionResults(firstConv?.id!!)
                    getConversionResults(secondConv?.id!!)
                }
                else{
                    with (prefMgr?.edit()) {
                        this?.putInt("wmft", pos)
                        this?.apply()
                    }
                    pViewModel._wmftpos = pos
                    pViewModel._wmsdpos = spinner2?.selectedItemPosition!!
        ...
        spinner2?.onItemSelectedListener = (object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, vw: View?, pos: Int, id: Long) {
                if(!pViewModel._swmspinInit){
                    pViewModel._swmspinInit = true
                    getConversionResults(firstConv?.id!!)
                }
                else{
                    with (prefMgr?.edit()) {
                        this?.putInt("wmsd", pos)
                        this?.apply()
                    }
                    pViewModel._wmftpos = spinner?.selectedItemPosition!!
                    pViewModel._wmsdpos = pos
