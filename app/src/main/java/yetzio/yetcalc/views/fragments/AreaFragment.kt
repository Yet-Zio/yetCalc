...
class AreaFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val initPos1 = prefMgr?.getInt("areaft", 0)!!
        val initPos2 = prefMgr?.getInt("areasd", 0)!!
        spinner?.setSelection(initPos1)
        spinner2?.setSelection(initPos2)
        pViewModel._areaftpos = initPos1
        pViewModel._areasdpos = initPos2
            ...
            try {
                when(id){
                    R.id.et_firstConversion -> {
                        val res = UnitConv.Area.convert(pViewModel._areaftpos, pViewModel._areasdpos
                    ...
                    R.id.et_secondConversion -> {
                        val res = UnitConv.Area.convert(pViewModel._areasdpos, pViewModel._areaftpos
    ...
    fun setupSpinner(){
        activity?.let {
            if(pLight){
                ...
                ArrayAdapter.createFromResource(it, R.array.arealist, R.layout.spinner_item)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
        ...
        spinner?.onItemSelectedListener = (object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, vw: View?, pos: Int, id: Long) {
                if(!pViewModel._areaspinInit){
                    pViewModel._areaspinInit = true
                    getConversionResults(firstConv?.id!!)
                    getConversionResults(secondConv?.id!!)
                }
                else{
                    with (prefMgr?.edit()) {
                        this?.putInt("areaft", pos)
                        this?.apply()
                    }

                    pViewModel._areaftpos = pos
                    pViewModel._areasdpos = spinner2?.selectedItemPosition!!
        ...
        spinner2?.onItemSelectedListener = (object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, vw: View?, pos: Int, id: Long) {
                if(!pViewModel._sareaspinInit){
                    pViewModel._sareaspinInit = true
                    getConversionResults(firstConv?.id!!)
                }
                else{
                    with (prefMgr?.edit()) {
                        this?.putInt("areasd", pos)
                        this?.apply()
                    }

                    pViewModel._areaftpos = spinner?.selectedItemPosition!!
                    pViewModel._areasdpos = pos
