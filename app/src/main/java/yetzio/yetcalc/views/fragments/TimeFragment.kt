...
class TimeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val initPos1 = prefMgr?.getInt("timeft", 0)!!
        val initPos2 = prefMgr?.getInt("timesd", 0)!!
        spinner?.setSelection(initPos1)
        spinner2?.setSelection(initPos2)
        pViewModel._timeftpos = initPos1
        pViewModel._timesdpos = initPos2
            ...
            try {
                when(id){
                    R.id.et_firstConversion -> {
                        val res = UnitConv.Time.convert(pViewModel._timeftpos, pViewModel._timesdpos
                    ...
                    R.id.et_secondConversion -> {
                        val res = UnitConv.Time.convert(pViewModel._timesdpos, pViewModel._timeftpos
    ...
    fun setupSpinner(){
        activity?.let {
            if(pLight){
                ...
                ArrayAdapter.createFromResource(it, R.array.timelist, R.layout.spinner_item)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
        ...
        spinner?.onItemSelectedListener = (object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, vw: View?, pos: Int, id: Long) {
                if(!pViewModel._timespinInit){
                    pViewModel._timespinInit = true
                    getConversionResults(firstConv?.id!!)
                    getConversionResults(secondConv?.id!!)
                }
                else{
                    with (prefMgr?.edit()) {
                        this?.putInt("timeft", pos)
                        this?.apply()
                    }
                    pViewModel._timeftpos = pos
                    pViewModel._timesdpos = spinner2?.selectedItemPosition!!
        ...
        spinner2?.onItemSelectedListener = (object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, vw: View?, pos: Int, id: Long) {
                if(!pViewModel._stimespinInit){
                    pViewModel._stimespinInit = true
                    getConversionResults(firstConv?.id!!)
                }
                else{
                    with (prefMgr?.edit()) {
                        this?.putInt("timesd", pos)
                        this?.apply()
                    }
                    pViewModel._timeftpos = spinner?.selectedItemPosition!!
                    pViewModel._timesdpos = pos
