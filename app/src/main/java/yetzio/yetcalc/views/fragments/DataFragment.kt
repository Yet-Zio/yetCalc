...
class DataFragment : Fragment() {
            ...
            try {
                when(id){
                    R.id.et_firstConversion -> {
                        val res = UnitConv.Data.convert(pViewModel._dataftpos, pViewModel._datasdpos
                    ...
                    R.id.et_secondConversion -> {
                        val res = UnitConv.Data.convert(pViewModel._datasdpos, pViewModel._dataftpos
    ...
    fun setupSpinner(){
        activity?.let {
            if(pLight){
                ArrayAdapter.createFromResource(it, R.array.datalist, R.layout.spinner_itemlight)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
            else{
                ArrayAdapter.createFromResource(it, R.array.datalist, R.layout.spinner_item)
                    .also { adapter ->
                        spinner?.adapter = adapter
                    }
            }
